...
{
    i32 res = ( equalf(l.position.x, r.position.x, 0.01f) && equalf(l.position.y, r.position.y, 0.01f) && equalf(l.position.z, r.position.z, 0.01f)  && equalf(l.normal.x, r.normal.x, 0.01f) && equalf(l.normal.y, r.normal.y, 0.01f) && equalf(l.normal.z, r.normal.z, 0.01f) && equalf(l.tex_coord.x, r.tex_coord.x, 0.01f) && equalf(l.tex_coord.y, r.tex_coord.y, 0.01f));
    return res;
}
...
static MeshData read_collada(String filepath)
{
   ...
   while(true)
   {
    ...
    i32 * indices = (i32*)arena_alloc(&global_platform.permanent_storage, sizeof(i32) * data.vertex_count); //not all this space is going to be used tho
    u32 indices_size = 0;

    vertex* unique_verts = (vertex*)arena_alloc(&global_platform.permanent_storage, sizeof(vertex) * data.vertex_count);
    u32 unique_verts_count = 0;

    for (u32 i = 0; i < data.vertex_count; ++i)
    {
        vertex vert = data.verts[i];
        //check if the vertex is already in array
        u32 j;
        for (j = 0; j < unique_verts_count; ++j)
        {
            //if a unique vertex already exists we index it
            vertex to_check = unique_verts[j];
            if (vert_equals(vert,to_check))
            {
                indices[indices_size++] = j;
                break;
            }
        }
        //the vertex is unique thus far, we add it
        if (j == unique_verts_count)
        {
            indices[indices_size++] = unique_verts_count;
            unique_verts[unique_verts_count++] = vert;
        }
...
static MeshData read_collada_old(String filepath)
{
   MeshData data = {0}; 
